...
call_instance = Call(id=123, scheduled_to=call_input.scheduled_to)
...
def test_create_call_schedules_task(call_service, call_repository, worker):
...
def test_create_call_returns_instance(call_service, call_repository):
    call_repository.create.return_value = call_instance
    result = call_service.create_call(call_input)
    assert result == call_instance


def test_make_call_fetches_instance(call_service, call_repository):
...
def test_make_call_requests_call(
    call_service, call_repository, twilio_adapter, settings
):
...
def test_make_call_updates_call_instance(call_service, call_repository, twilio_adapter):
    call_repository.find_by_id.return_value = call_instance
    twilio_adapter.create_call.return_value = {"sid": "abc123"}
    call_service.make_call(call_instance.id)
    call_repository.save.assert_called_once_with(call_instance)
